...
repo_dir = fullfile(function_dir, '..', '..', '..');
data_dir = fullfile(repo_dir, 'data', 'form_factors');

    function [f_1, f_2] = find_f1f2(Z, energy_source)

        % read in database BrennanCowanLong
        fid          = fopen(fullfile(data_dir, 'f1f2_BrennanCowanLong.dat'));
...
file = importdata(fullfile(data_dir, 'ASF.DAT'));
