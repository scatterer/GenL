...
% Description:      generate density and propagate matrix
%
...
if ~full_stack 
  ...
  if stack{o}.roughness
    ...
    if control.pol == 0 || control.pol == 1
      output.refl   = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_e_rough,dz,substrate_end,nvector,B,control.pol,control.roughness_averaging);
    elseif control.pol == 2
      refl_p   = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_e_rough,dz,substrate_end,nvector,B,0,control.roughness_averaging);
      refl_s   = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_e_rough,dz,substrate_end,nvector,B,1,control.roughness_averaging);
    ...
    if control.pol == 0 || control.pol == 1
      output.refl = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_restr,dz,stack{o}.sigma,control.pol,control.roughness_averaging);

    elseif control.pol == 2
      refl_p = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_restr,dz,stack{o}.sigma,0,control.roughness_averaging);
      refl_s = propagate_vectorized_chunks_opt(Q,lambda,rho_0r,rho_1r,round(stack{1}.N),rho_restr,dz,stack{o}.sigma,1,control.roughness_averaging);
