...
outputs are unchanged. `Save plots...` exports the displayed cost, diffraction,
and electron-density panels as PNG, JPEG, TIFF, SVG, or PDF. `Export graph
data...` writes the diffraction data, fit, and residual as CSV or tab-delimited
text, plus a separate `_density` table when an electron-density profile is
available. The left side keeps the run setup, Run/Stop buttons, status,
