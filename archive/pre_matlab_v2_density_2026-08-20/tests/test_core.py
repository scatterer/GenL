...
class CoreTests(unittest.TestCase):
    ...
    def test_fe_v_stack_expands_and_simulates(self):
        ...
        self.assertEqual(
            len(layers),
            1 + int(sequence["repetitions"]) * len(sequence["layers"]),
        )
        self.assertEqual([layers[1].n, layers[2].n], [2, 14])
        self.assertEqual(
            [Path(layer.filename).stem for layer in layers[1:5]],
            ["Fe_fractional", "V_fractional", "Fe_fractional", "V_fractional"],
        )
    ...
    def test_superlattice_fit_residual_uses_selected_parameters(self):
        ...
        interface = float(document["sequence"]["layers"][0]["dinterface"])
    ...
    def test_superlattice_capping_layer_is_appended_and_fittable(self):
        ...
        self.assertEqual(len(layers), len(base.layers()) + 1)
