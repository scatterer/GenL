...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval] = deopt(fname,plotname,S_struct)

% if GUI is used, flag is used to stop the fitting
if S_struct.GUI == 1
    global flag_stopped;
    global progress;
    global gofs;
    gofs = zeros(S_struct.I_itermax,1);
end
...
FM_pop(I_best_index,:) = S_struct.GKP_bestmem;

S_val(1)  = feval(fname,FM_pop(I_best_index,:),S_struct);
S_struct = S_val(1).S_struct;
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0)
     ...
     if S_bestval.FVr_oa(1) ~= oldbest
       if ((rem(I_iter,I_refresh) == 0) || I_iter == 1)
         ...
         % end
         if (I_plotting == 1)
            feval(plotname,FVr_bestmem,I_iter,S_struct); 
         end
        
         % if Code is running WITH GUI
          if S_struct.GUI == 1
              fprintf(1,'Progress: %d Percent\n', progress);
          end
       end
      oldbest = S_bestval.FVr_oa(1);
     end
  ...
  if S_struct.GUI == 1
    gofs(I_iter) = S_bestval.FVr_oa(1);
  end
  
  progress = round(I_iter/S_struct.I_itermax*100);
